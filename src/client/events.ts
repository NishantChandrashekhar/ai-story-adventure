...
class EventHandler {
  ...
  setupChoiceButtons() {
    if(!ui.elements.choicesContainer) return;
    ui.elements.choicesContainer.addEventListener('click', async (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if(!target || !target.classList.contains('choice-btn')) return;
      const choiceText = target.textContent;
      if (!choiceText) return;
      await this.handleChoiceClick(choiceText);
    });
  }

  // Setup form submission
  setupFormSubmission() {
    if (!ui.elements.form) return;
  ...
  setUpNewStoryButton(){
    if (!ui.elements.newStoryButton) return;
  ...
  setUpClearChatButton(){
    if (!ui.elements.clearChatButton) return;
  ...
  async handleChoiceClick(choice: string) {
    try {
      // Update state
